    ...
    <div class="container">
      ...
      <div id="navbar" class="collapse navbar-collapse">
        <ul class="nav navbar-nav navbar-right">
          <li>
            <a href="./index.php"><span class="glyphicon glyphicon-user"></span> 平台首页</a>
          </li>
...
if(isset($_POST['submit'])) {
	foreach ($_POST as $k => $value) {
		...
		$DB->query("insert into vip_config set `k`='{$k}',`v`='{$value}' on duplicate key update `v`='{$value}'");
	}
	$pwd=daddslashes($_POST['pwd']);
	if(!empty($pwd))$DB->query("update `vip_config` set `v` ='{$pwd}' where `k`='admin_pwd'");
	showmsg('修改成功！',1);
}else{
?>
...
<div class="panel-body">
  <form action="./set.php" method="post" class="form-horizontal" role="form">
	...
	<div class="form-group">
	  ...
	  <div class="col-sm-10"><textarea class="form-control" name="cd" rows="2"><?php echo htmlspecialchars($conf['cd']); ?></textarea></div>
	</div><br/>
	<div class="form-group">
	  <label class="col-sm-2 control-label">公告</label>
	  <div class="col-sm-10"><textarea class="form-control" name="gg" rows="3"><?php echo htmlspecialchars($conf['gg']); ?></textarea></div>
	</div><br/>
	<div class="form-group">
	  <label class="col-sm-2 control-label">接口</label>
	  <div class="col-sm-10"><textarea class="form-control" name="jk" rows="3"><?php echo htmlspecialchars($conf['jk']); ?></textarea></div>
	</div><br/>
	<div class="form-group">
	  <label class="col-sm-2 control-label">统计代码</label>
	  <div class="col-sm-10"><textarea class="form-control" name="tj" rows="3"><?php echo htmlspecialchars($conf['tj']); ?></textarea></div>
	</div><br/>
	<div class="form-group">
